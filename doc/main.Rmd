...
Based on user database which contains the history of users' behaviours or preferance expressed by a set of votes vi,j corresponding to the vote for user i on item j, Memory-Based Algorithm predict the votes of the active users on items new to them based on a set of weights between users calculated from the user database.
It can deal with both implicit data(MS website), which is binary, showing that whether a user voted or not, and explicit data(Movie), showing the scores. By using default voting, we first used different methods to calculate correlations between users(pearson, spearman, pearson+variance weight, SimRank), and then defined a set of neighbors which have specific correlations with an active user by setting weight thresholds.
For prediction part, predicted scores are assigned to the items users never voted before. For evaluation part, the implicit data used expected utility of a ranked list of items to show how users are satisfied with the recommendation and the explicit data used MAE to get the accuracy.
During the process, besides using different methods to calculate correlation, different thresholds which showed different coverage and the way to calculate predicted scores(scaled or not) are also two variables in this algorithm.
#### Stage
```{r}
transfer_1 <- FALSE
...
for(i in 1:4){
  ...
  var_pear_score_noweighted[i]<-rank_score(pred_var_pear_1_noweighted,df_test1)
    ...
    # paste0("By using pearson correlation with threshold ",k[i],", the expected utility of the ranked list is ",rank_score(pred_pearson_1,df_test1))
  # paste0("By using spearman correlation with threshold ",k[i],", the expected utility of the ranked list is ",rank_score(pred_spearman_1,df_test1))
  # paste0("By using variance weight(pearson) with threshold ",k[i],", the expected utility of the ranked list is ",rank_score(pred_var_pear_1,df_test1))
  # paste0("By using SimRank with threshold ",m[i],", the expected utility of the ranked list is ",rank_score(pred_simrank_1,df_test1))
}
