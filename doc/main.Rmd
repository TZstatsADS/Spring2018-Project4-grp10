...
if(transfer_1 & transfer_1_test & transfer_2){
  ...
  df_test1<-transfer_1_test(test1,df.train1)
...
#### get weight matrix and select neighbors by threshold
```{r}
if(sim_weight & var_weight & simrank){
  weight_pearson_1<-sim_weight(df_train1,"pearson")
  weight_spearman_1<-sim_weight(df_train1,"spearman")
  weight_pearson_2<-sim_weight(df_train2,"pearson")
  weight_spearman_2<-sim_weight(df_train2,"spearman")
  ...
  weight_var_1<-var_weight(df_train1,"")
  weight_var_2<-var_weight(df_train2,"")
  save(weight_var_1, file = paste0("../output/weight_var_1.RData"))
  save(weight_var_2, file = paste0("../output/weight_var_2.RData"))
  weight_simrank_1<-simrank(df_train1)
  weight_simrank_2<-simrank(df_train2)
  save(weight_simrank_1, file = paste0("../output/weight_simrank_1.RData"))
  save(weight_simrank_2, file = paste0("../output/weight_simrank_2.RData"))
}else{
  ...
  load("../output/weight_var_1.RData")
  load("../output/weight_var_2.RData")
  load("../output/weight_simrank_1.RData")
  load("../output/weight_simrank_2.RData")
}

k=0.5
neighbors_pearson_1<-get_neighbors_index(weight_pearson_1,k)
neighbors_pearson_2<-get_neighbors_index(weight_pearson_2,k)
neighbors_spearman_1<-get_neighbors_index(weight_spearman_1,k)
neighbors_spearman_2<-get_neighbors_index(weight_spearman_2,k)
neighbors_var_1<-get_neighbors_index(weight_var_1,k)
neighbors_var_2<-get_neighbors_index(weight_var_2,k)
neighbors_simrank_1<-get_neighbors_index(weight_simrank_1,k)
neighbors_simrank_2<-get_neighbors_index(weight_simrank_2,k)
```

#### Calculate the prediction score and evaluate the ranked list(for MS data)
```{r}
pred_pearson_1<-pred_1(df_train1,df_test1,weight_pearson_1,neighbors_pearson_1$top.neighbors)
pred_spearman_1<-pred_1(df_train1,df_test1,weight_spearman_1,neighbors_spearman_1$top.neighbors)
pred_var_1<-pred_1(df_train1,df_test1,weight_var_1,neighbors_var_1$top.neighbors)
pred_simrank_1<-pred_1(df_train1,df_test1,weight_simrank_1,neighbors_simrank_1$top.neighbors)

paste0("By using pearson correlation with threshold ",k,", the expected utility of the ranked list is ",rank_score(pred_pearson_1,df_test1))
paste0("By using spearman correlation with threshold ",k,", the expected utility of the ranked list is ",rank_score(pred_spearman_1,df_test1))
paste0("By using variance similarity with threshold ",k,", the expected utility of the ranked list is ",rank_score(pred_var_1,df_test1))
paste0("By using SimRank with threshold ",k,", the expected utility of the ranked list is ",rank_score(pred_simrank_1,df_test1))
```

```{r}
pred_pearson_2<-pred_2(df_train2,df_test2,weight_pearson_2,neighbors_pearson_2$top.neighbors)
pred_spearman_2<-pred_2(df_train2,df_test2,weight_spearman_2,neighbors_spearman_2$top.neighbors)
pred_var_2<-pred_2(df_train2,df_test2,weight_var_2,neighbors_var_2$top.neighbors)
```
